...
- **調査=read / 修正=write の構造分離**。修正だけは API サーバの外に出す。既定（`advisory`）は方針テキストの **draft PR**、`dispatch` では GitHub Actions 上で AI が実コードを修正し Trivy 再スキャン＋テスト緑を通って draft PR——どちらも自動マージなし・人間承認ゲート（**dispatch 経路の実行実績はまだ 0**。既存の draft PR は advisory 経路のもの → [architecture.md §5](docs/architecture.md)）。
