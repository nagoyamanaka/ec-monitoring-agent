...
| 実測（デモ環境）   |                                                                                           |
| ------------------ | ----------------------------------------------------------------------------------------- |
| 予報               | 未来シグナル3系統を過去の記憶と突合（記憶は確度の増幅材）・**引用は実在照合済みのみ表示** |
| 予報の届け先       | 画面に加え、**該当する PR のコメント**へ（リリース判断の場に材料を置く・**止めない**）    |
| 未知障害の AI 調査 | **ADK 8エージェント**が read-only 横断・約2〜3分で根拠リンク付きレポート                  |
| 既知障害の判定     | **1秒未満・AI コストゼロ**（決定論）                                                      |
| 測定（母数つき）   | 引用照合率 **9/9**（本番 Alert 10件）・予報の偽引用破棄 **0/155**（ローカル21回。本番31回でも破棄0）＝**n が小さいので率としては主張しない** |
| テスト             | ユニット **1,298件** ＋ E2E 22件（学習ループ・引用検証を決定論担保）                      |

## 何が違うか

- **起きる前に、根拠付きで予報する（予兆ブリーフィング・実装済み）**。未マージ PR・未適用 Terraform plan・負荷スケジュールという「未来シグナル」を過去の障害の記憶と Gemini が突合し、「土 20:00、DB 接続枯渇 高リスク」のように予報（**過去の同型障害は確度を上げる材料であって発火の関門ではない＝前例が無くても未来シグナル単独で予報は出る**。⚠ 記憶が無いと弱く出る、ではない——ローカル実測 n=21 で記憶ゼロでも9回中9回が最上位（高リスク）。変わるのは段の高さではなく**1回あたりの本数**〈2.25→1.67本〉）。引用は実在シグナルと機械照合し**偽の引用は表示前に自動破棄**。各リスクには「🛡 今打てる先手」を1行提示（実行主体は人間・write ゼロ）。カードは**決裁の読み順**（何が → いつまでに → 何をする → なぜ → 証拠）で、「対処を始める期限」を時間軸で出す。
- **予報を、決裁の場に置く（実装済み）**。PR が開くと該当する予報が **PR コメント**として出る——level・根拠 N種類・実在照合済みの引用・今打てる先手・「あと N 時間／対処は約30分（宣言値）」の有効リードタイム付き。**ただし gate にはしない**（止める実装を入れない・該当が無ければ黙る・失敗しても CI は落とさない）。予測発生時刻は引用されたスケジュールから決定論で解決＝**人間が日時を入力する箇所はゼロ**。→ [architecture.md §10.9](docs/architecture.md)
...
- **調査=read / 修正=write の構造分離**。修正だけは API サーバの外に出す。既定（`advisory`）は方針テキストの **draft PR**、`dispatch` では GitHub Actions 上で AI が実コードを修正し Trivy 再スキャン＋テスト緑を通って draft PR——どちらも自動マージなし・人間承認ゲート（**dispatch 経路は本番で実測済み**——UI の起票ボタン → `repository_dispatch` → ランナー上で修正・テストゲート通過 → draft PR → callback で確定まで 9分33秒 → [architecture.md §5](docs/architecture.md)）。
- **ドッグフーディング（自己運用ループ）**: このリポジトリ自身の CI（Trivy）の検出が本番の `/ingest/security-scan` に流れ、SECURITY 調査が自リポジトリへの draft PR を起こす。**起きる前の側も同じループに入っており**、自リポジトリの PR には自分が出した予報がコメントとして届く＝**監視対象の EC も、監視するエージェント自身も、同じ DevOps ループの中にいる**（`.github/workflows/` の実ワークフローが運用系そのもの。図解 → [architecture.md §6.5](docs/architecture.md#65-devops-ドッグフーディング自己運用ループ)）。
...
![Kizashi システム全体の処理フロー — 検知（既存基盤）→ ingest 3系統 → 分類（既知1秒/類似/未知）→ AI 調査（ADK 8エージェント・read-only）→ 人間レビュー→学習ループ、起きる前の予兆ブリーフィングと決裁の場（GitHub PR コメント）への提示、write 隔離の AI 修正 draft PR](docs/assets/architecture.png)
...
flowchart LR
  ...
  PRC["GitHub PR コメント<br/>決裁の場へ材料を届ける（止めない）"]
