...
/** カードが組み立てた軸を渡す形になったので、テストも同じ経路で組み立てる。 */
function Mounted({ risk, generatedAt }: { risk: RiskCardView; generatedAt: string }) {
  const timeline = buildForecastTimeline(risk, generatedAt);
  return timeline ? <ForecastTimeline timeline={timeline} risk={risk} /> : null;
}

describe("ForecastTimeline", () => {
  it("画面に無かった「対処を始める期限」を出す", () => {
    render(<Mounted risk={RISK} generatedAt={generatedAt} />);
  ...
  it("間に合わない予報は不足分を出す（負を隠さない）", () => {
    render(<Mounted risk={RISK} generatedAt="2026-08-08T10:50:00.000Z" />);
  ...
  it("スケジュールを引用していないリスクでは軸ごと出さない（縮退）", () => {
    const { container } = render(
      <Mounted risk={{ ...RISK, citations: [] }} generatedAt={generatedAt} />,
    );
