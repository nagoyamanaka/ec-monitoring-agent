 ...
 *
 * グルーピングは「AI がどう関与するか」の軸で2群に分ける（＝ポートフォリオ/ハッカソン評価の肝）:
 *
 *  既知障害 … 過去に AI が調査・承認した既知パターンに完全一致。分類と重大度が即座に確定し、
 *      調査レポートは AI が**今回の具体パラメータ**で生成する（既知パターンを grounding 文脈に
 *      渡すので高速・高信頼に収束）。同一 dedupKey の重複観測は畳み込み、発生回数だけ加算する。
 *  未知障害 … 既知に一致しない障害。**ADK マルチエージェント**が起動し、実ログ・実コミット差分を
...
const SCENARIO_GROUPS: readonly ScenarioGroup[] = [
  {
    title: "既知障害 ― 学習済みパターンで高速収束",
    aiRole:
      "過去に AI が調査・承認した既知パターンに完全一致。分類と重大度を即座に確定し、調査レポートは AI が今回の具体パラメータで生成（既知パターンを文脈に grounding）。重複観測は dedup で畳み込み発生回数を加算。",
    scenarios: [
      {
        id: "1",
        label: "決済タイムアウト",
        description: "決済が時間切れ → 既知パターンに完全一致 → AI が今回の値で報告生成",
        realness: "live",
      },
      {
        id: "2",
        label: "在庫不足",
        description: "在庫が足りず予約失敗 → 既知パターンに完全一致",
        realness: "live",
      },
    ],
  },
  ...
  return (
    <div className="space-y-2">
      ...
      <p className="rounded-md bg-slate-800/40 px-2.5 py-2 text-[11px] leading-relaxed text-slate-300 ring-1 ring-inset ring-slate-700/50">
        ...
        （入口だけ合成・パイプラインは実 AI）。既知/未知ともレポートは AI が今回の
        パラメータで生成し（既知はパターンを文脈に grounding）、未知は ADK
        マルチエージェントが実ログ・実コミット差分を引用して調査、承認で知識ベースに昇格する。
      </p>
    </div>
  );
}
