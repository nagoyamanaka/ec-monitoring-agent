 ...
 * - 受信ごとに onAlert / onRemediation / onInvestigationProgress を呼ぶ。state マージ／同一 ID 置換は
 *   呼び出し側が純関数で行う
...
export function useAlertStream(
  ...
  const onInvestigationProgressRef = useRef(onInvestigationProgress);
  onInvestigationProgressRef.current = onInvestigationProgress;
  ...
  useEffect(() => {
    const unsubscribe = stream.subscribe(
      ...
      (progress) => onInvestigationProgressRef.current?.(progress),
    );
    return unsubscribe;
  }, [stream]);
}
