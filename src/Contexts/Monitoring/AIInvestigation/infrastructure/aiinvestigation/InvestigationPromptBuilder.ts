...
関連は「具体的な共有証拠」で裏付けられる場合のみ載せること＝共通の infraEvidence シグナル（同一の
...
confidence（0〜1）は「結論をどれだけ証拠で裏付けられたか」の中立な自己評価です。大きく見せるための値ではありません。
実際に積み上げた証拠の強さだけを反映し、過大評価しないこと。次の較正基準に従ってください:
- 0.85〜0.95: 直接証拠で因果がほぼ確定（例: 直近 commit/terraform 差分が症状と時系列・内容で一致し、メトリクスの裏付けもある）。
- 0.55〜0.75: 状況証拠・相関のみで因果は妥当だが断定はできない（複数の仮説のうち最有力だが決定打を欠く）。
- 0.40 未満: 証拠が薄い／競合する仮説が残る／未知パターンで切り分け不能。この場合は summary でも断定せず
  「確証には〇〇の証拠が必要」と不足を明示し、impact.fault は "unknown" にすること。
証拠が足りないのに高い confidence を出すこと（過信）は誤りです。分からないときは低い値を正直に出してください。
なお下の JSON 例の confidence 値（0.6）は出力形式の見本であり、目標値ではありません。
{
