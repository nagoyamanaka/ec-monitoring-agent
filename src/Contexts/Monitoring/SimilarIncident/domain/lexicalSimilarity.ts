...
// この有界な指標を使う**。BM25 の生スコアは無界で、同じ文書・同じクエリでもコーパス規模（IDF）で
// 尺度が伸び縮みする（事例が増えるほど大きくなる）ため、固定の除数と閾値では「少ないうちは本物が
// 届かず棄権、増えると無関係事例が閾値を超えて偽 KNOWN」のどちらかが必ず起きる。Jaccard なら構成上
// [0,1] に収まり、Elastic と InMemory が同じ意味の score を返せる。
//
