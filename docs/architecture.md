...
- F5d 予報台帳（測定計画 T0-1）: 予報の保存直後に、引用検証を通った **risk 1件ごとに `issued` を Mongo `forecast_ledger` へ追記**する（`ForecastLedgerRecorder`・`MongoForecastLedgerRepository`）。1行は `forecastId`（risk ごとに新規の Uuid）・`briefingId`（元の `RiskForecast.forecastId`）・`protocolVersion`（`MEASUREMENT_PROTOCOL_VERSION`・未設定は `unregistered`）・`forecasterVersion`（モデル名・システムプロンプト・閾値・クラス定義・コード上の規則一覧 `FORECAST_PIPELINE_RULES`/`FORECAST_OUTPUT_RULES` の sha256）・`subjectKey`（`RiskItem.subject` をそのまま）・`class`・`level`・`issuedAt`/`windowEndsAt`（UTC）・`windowLengthHours`・`windowPolicyVersion`（クラス別窓長の組をそのまま JSON 文字列で）・`evidenceSnapshotId`（入力の証拠スナップショット・F5e。保存に失敗した回と T0-2 以前の行は空）・`signalFingerprint`（引用シグナルの内容の集合ハッシュ・連番 id は除く）・`assignment`=`normal`/`assignmentProb`=1.0 を持つ。`class` は引用シグナルの種類から決定論的に決まる（FUTURE_CHANGE を含む→`change_risk`／SCHEDULE を含む→`load_risk`／MEMORY のみ→`recurrence_risk`）。窓長は既定 72h で `FORECAST_WINDOW_HOURS_BY_CLASS`（例 `{"load_risk":24}`）でクラス別に上書き。**追記専用**: コレクションは `insertOne`/`find`/`createIndex` だけに型で絞り、`issued` は `forecastId` の部分一意インデックスで二重発行を拒む。outcome は持たせず、決着（T1-1）は別イベントとして追記する。追記の失敗は `forecast_ledger_append_failed`（error）を残して予報の生成・配信は止めない。demo reset の対象外。
- F5e 証拠スナップショットとリプレイ（測定計画 T0-2）: 予報器を呼ぶ**直前**に、その入力（`ForecastContext`＝horizon＋主シグナル・MEMORY を結合済みのシグナル全量）を正準 JSON（キー順固定・undefined 除去・文字列とシグナル順と id はそのまま）に凍結し、その sha256 を `snapshotId` として Mongo `evidence_snapshots` へ保存する（`EvidenceSnapshotRecorder`・`MongoEvidenceSnapshotRepository`・`_id`＝snapshotId の内容アドレス。`content` は文字列で持ち、読み戻したバイト列がそのままハッシュ入力になる）。同じ入力は同じ id に畳まれ最初の `capturedAt`（UTC）が残る。台帳行の `evidenceSnapshotId` がこれを指す。保存の失敗は `evidence_snapshot_save_failed`（error）を残し、台帳には空文字を書いて予報は止めない。シグナル0件の回（予報器を呼ばない）は作らない。**リプレイ**は `pnpm replay --snapshot <id> --forecaster <version|current>`（`src/apps/backoffice/backend/src/replay.ts`）: snapshot と台帳行を読んで Mongo を閉じてから、`fetch`・`http(s)`・`net`/`tls` を「許可ホスト以外は例外」に差し替えて（許可は Gemini のエンドポイントと認証だけ・`AI_INVESTIGATION_STUB=true` なら許可ゼロ）、本番と同じ `ForecastSynthesis`（予報器＋引用検証）で再実行し、subject ごとの level を同じ `forecasterVersion` の台帳行と突き合わせた JSON を stdout に出す。遮断に当たった通信が1件でもあれば、予報器が例外を握りつぶして fallback を返していても失敗にする。動かせるのはこのチェックアウトの予報器だけ（`--forecaster` が今の版と違えば実行しない＝別の版はそのコミットをチェックアウトして動かす）。**非決定性**: Gemini 経路は temperature / seed を指定していない（モデル既定）ので同じ入力・同じ版でも level が揺れうる（出力の `determinism` に記録。凍結中の予報器は変えないので指定も足していない）。stub は固定出力で一致する。**保存量と保持**: 現行のデモ入力で 1件 約4.4KB（シグナル十数件）。内容アドレスなので同じ入力の再生成では増えず、1日数十回の生成でも月数 MB に収まる。OSS バックテスト（T1-2）で diff を含めると1件数十〜数百KB になりうるが、Mongo の1ドキュメント上限 16MB には遠い。台帳の参照先＝測定の標本なので削除しない（demo reset の対象外・TTL なし）。
