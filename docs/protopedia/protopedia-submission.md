...
**1. 起きる前に、根拠付きで予報する**
未マージ PR・未適用の Terraform plan・週末セールなどの負荷予定と、過去の障害の記憶を Gemini が突合し、「土 20:00、DB 接続枯渇 HIGH」のようにリスクを予報します。根拠の引用は実在するシグナルと照合して検証し、偽の引用は自動で落とします。予報には「いま打てる先手」も1行で添え、引用チップからそのまま対処先（実在の draft PR・過去事例）へ飛べます。
