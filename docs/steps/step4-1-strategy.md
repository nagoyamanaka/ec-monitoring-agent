...
**Elasticのシナジー**: 過去の解決済みインシデントへの**あいまいテキスト検索**（BM25 fuzzy + Jaccard類似度）。SimilarIncident が蓄積されるほど分類の確度が上がり、ハルシネーションでなく**引用付きの仮説**が出る。

> **Kibana/ELK との位置づけの違い**: ELK はログを「人間が掘る（pull 型）」可視化ツールで、検知・自動化ともにない。本プロダクトは検知後の「AI 自律調査→評価→レビュー起票」を1ループで圧縮する **push 型ワークフローエンジン**。「ログを見つけに行く」と「アラートが来たら AI が証拠を集めて人間に渡す」の違い。
