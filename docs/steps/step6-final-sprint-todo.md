...
- 【実地データ 2026-07-05・第6の原因＝思考トークンによる出力予算の食い潰し】デプロイ先でシナリオ 3b と 6 が揃って fallback: `elapsedMs≈46-60秒・timedOut=false・finalTextLen=0（rawLen=0＝切断ですらなく空）`、agentTrace は coordinator→root_cause_analyst の委譲あり＝グラフは動いている。真因: gemini-2.5-pro は**思考トークンも maxOutputTokens を消費**するため、I1 で切断防御に入れた `maxOutputTokens: 65535`（モデル上限）が逆に「思考が全予算を食える」余地になり、高推論シナリオ（3b=インフラ因果連鎖・6=コード退行分析）の最終 JSON 合成ターンで **finishReason=MAX_TOKENS・回答 0 文字**。空文字は salvage（`indexOf("{")=-1`）も救えず fallback 直行。**07-02 の「第3の原因」（events=1・59秒・空＝無言ドロップ疑い）も同一メカニズムで説明がつく**（初回ターンの思考が予算を食い切った形）。✅ 対策実装（2026-07-05）: ① coordinator に `thinkingConfig: { thinkingBudget: 16384 }`（pro の有効域 128〜32768・回答用に約49K を必ず残す）② **対策(a) sub-agent flash 化を静的ロール割当で実装**: `AI_INVESTIGATION_COLLECTOR_MODEL`（evidence_collector・既定 flash＝ツール往復が主で最多呼び出し）・`AI_INVESTIGATION_ESCALATION_MODEL`（runbook_escalation・既定 flash＝マスタ引き＋定型草案）・`AI_INVESTIGATION_TRIAGE_MODEL`（impact_triage・**既定は主モデル**＝citation 必須ガード直結のため flash 化は計測してから）。ルーターLLM は不採用（+1 呼び出しの遅延・非決定性・ロール自体が難易度の代理変数）。per-alert の予算指定はクライアント入力にせず、必要になれば分類結果からサーバ側で決定的に導出する方針（いたずらリスク排除）。③ **配線境界の確定**: ADK 調査は **worker（GCE backbone）専任**（edge=Cloud Run は `consumeEvents = role!=="edge"` で RabbitMQ 非購読・`AI_INVESTIGATION_ADK` も terraform 未設定＝単一Gemini）。よって coordinator thinkingBudget は docker-compose.prod.yml（worker）のみ配線し **terraform/main.tf はノータッチ**。ただし **edge の再調査経路（POST /alerts/:id/reinvestigate）は in-process で単一Gemini（`GeminiLLMClient`）を叩く別アダプタ**で、これも `maxOutputTokens`/思考未キャップで同型の空応答リスクがあったため ④ **`GeminiLLMClient` にも同じ防御**（`maxOutputTokens:65535`＋`thinkingConfig:{thinkingBudget: GEMINI_THINKING_BUDGET(既定16384)}`）を適用。`GEMINI_THINKING_BUDGET` は `GEMINI_TIMEOUT_MS` と同じくコード既定＝image 経由で edge に届く（terraform 非掲載の慣例に合わせる）。**実走確認（3b/6 で finalTextLen>0 に戻ること・flash 化で citation 品質が落ちないこと・edge 再調査でも空応答が出ないこと）は人間**
...
- [x] 【追撃3 2026-07-07・シナリオ4はカタログ側の欠落＝CVE を正式引用種別に昇格】シナリオ4（security-vuln）で全件未照合: 引用が `errorEvent.payload.vulnerabilities[0].cveId:CVE-2021-3807` 等のJSONパス装飾。真因はプロンプトでなく**カタログの欠落**＝このシナリオの本質的証拠（CVE ID）が payload にしか無く、citableIds に「引用できるID」が存在しなかった（根拠を出せない→発明する、の再演）。対策＝**CVE を照合カタログの正式種別に昇格**: `CitationSourceKind` に `cve` 追加／`buildCitationCatalog` が payload を深さ制限付きで走査し **値全体が `CVE-\\d{4}-\\d{4,}` の文字列のみ**を収集（orderId 等の生データUUIDは載せない一線を維持・CVE は NVD 決定論リンク＝検証可能IDで verifiable_cve 強シグナル/SecurityFindingView と同じ判断）・大文字正規化・`https://nvd.nist.gov/vuln/detail/{id}` を href に／frontend は `CITATION_KIND_LABEL`「脆弱性 (CVE)」・観測データレーンに写像（Record 型で網羅コンパイル強制）。citableIds には自動で載る（同一ソース）。装飾付き引用も包含一致で cve に解決することをUTで固定。backend 207＋frontend 365＋両tsc 緑
- [x] 【追撃4 2026-07-07・プロンプト非依存の最終防御＝充填トークンのパース時除去】`LLMOutputParser` の引用系4取り込み口（relatedAlerts.citations／impact.citations／escalation.evidenceBundle／remediationReview.citations）を共通の `toCitationArray` に統一し、定型の充填トークン（`N/A`・`none`・`null`・`-`・`なし`・`特になし`・case-insensitive）を決定論で除去。**未照合の実引用は落とさない**（正直さの担保は表示側のまま）＝落とすのは「引用の体裁をした空値」だけ。充填のみになった impact は既存のマッパ空 citations ガードで丸ごと落ちる＝「N/A は根拠なし」と同義に収束。preventiveAction の safeParse drop と同じ防御的正規化の流儀。stub 調査出力は impact 無し＝未照合レーン自体が出ないため対応不要を確認。UT追加・**backend 全1040＋frontend 365＋両tsc 緑**
- [ ] 【確認・人間】実 Gemini 再生成（F8/D3 実走と同一セッションで可）で ✓照合済みチップが出ること・未照合が例外側に落ちること（経緯: 記述縛り版でシナリオ2/3b全件✓→シナリオ1未照合1件→シナリオ1全件✓→シナリオ2で装飾/N-A引用→citableIds 列挙版へ→シナリオ4でCVE欠落→カタログにcve追加＋充填除去。**citableIds＋cve＋充填除去版で全シナリオ（1/2/3b/4）を再確認**。なお照合の部分一致規則により装飾付きでも実IDを含めば✓になる＝列挙版はその揺れ自体を抑える狙い）
...
### タスク E9: レビュー判定フィードバックの対称化＋判定グリフの SVG 化 〔実装✅ 2026-07-05〕

**問題（実機フィードバック）**: ①承認は「承認を記録しました。学習シグナルとして残ります」と語るのに、却下のみ（記録だけ）は `却下理由：「あ」` の裸 echo だけ＝記録が何に効いたか無言（**backend は実際に効いている**: 誤分類として Analytics 分類正答率の母数に入る＋承認→却下の訂正時は類似学習・自動昇格を撤回する `withdrawResolved`＝学習ループの信頼性の見せ場が黙殺されていた）。②テキスト記号 ✓/✗ がフォント依存で安っぽい。

- [x] 却下の完了通知（承認と対の rose 版・`AlertReviewPanel`）: 「却下を記録しました。AI の分類が誤りだったことが学習シグナルとして残ります（Analytics の分類正答率に反映）」＋**承認→却下の訂正時のみ**「承認時に積まれていた類似学習・自動昇格は撤回しました」（送信前の reviewState で判別・backend の `withdrawResolved` と正直に対応）。通知は decide() が判定のたびセット/クリア＝refetch 前の窓でも消えない
- [x] 永続表示の格上げ: `却下理由：「…」` → 「却下済み — 誤分類として記録されています（分類正答率に反映）」ラベル＋「指摘メモ：「…」」の2行（セッション通知表示中は重複させない）
- [x] 判定グリフの SVG 化: `shared/ui/DecisionIcon`（CheckCircle/XCircle/AlertTriangle・stroke ベース手組み・currentColor 継承）を新設し、`AlertReviewPanel`（承認/却下トグル・完了通知・昇格）と `RemediationReviewPanel`（pass/concerns/reject 判定ピル）のテキスト記号 ✓/⚠/✗ を置換。ドロワー✕（閉じる）・FilterChip の ✓✕ は**判定語彙でない**ため対象外
- 「却下のみ」が記録だけである主従（主役＝却下して AI 再調査）・却下を SimilarIncident に積まない（誤答を記憶に混ぜない）方針は不変。RTL 3件追加＋既存アサート追従・frontend 312 全緑
