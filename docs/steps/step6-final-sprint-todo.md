...
- 【バグ】チップ集計とカードバッジの状態算出を単一ソース化（レビュー待ち件数が一覧と常に一致）
...
- 空状態に CTA: 「→ 右のデモシナリオから障害を注入してください」＋確度スペクトル3群の1行説明（デモ卓への視線誘導）
- 取得失敗時: 自動リトライ（指数バックオフ・n回）＋「起動処理中の可能性があります。自動で再試行しています…」の文言。生の `HTTP 500` は詳細折りたたみへ
- 統計タイル「アラート」→「アクティブアラート」等、一覧と同じ軸のラベルに統一
- 初回訪問ガイド（dismissible・3ステップ: ①注入 → ②AI調査を見る → ③承認で学習）。localStorage で1回きり
