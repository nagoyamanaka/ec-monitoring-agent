...
| [ADR-14](#adr-14-リアルタイム反映はsse-push--pull-on-demandの1軸で割る) | SSE push / pull の1軸 | フロント |
...
| [ADR-26](#adr-26-空応答-fallback-は思考予算を落とした縮退リトライ1回で防御) | 空応答 fallback: 縮退リトライ1回 | AI |
...
- 🚨 **有効リードタイムの予測発生時刻は「引用されたスケジュール」から解決する。人間には入力させない**（当初 CI の repo variable で受ける実装にしたが、**設計として誤りだったので差し替えた**）。誤りの中身は3つ: (1) **所有権の反転**——「いつ負荷が来るか」は製品が持っている情報（`SCHEDULE` シグナル・実体は `FORECAST_SCHEDULE_SEED`）で、人間が CI 設定へ転記するものではない (2) **決裁の場に出す数字の出所が人間になる**——手入力の定数から出たリードタイムは、計算しているように見えて実質は申告値で「盛らない」規律に反する (3) **静かに腐る**——固定日時は過ぎた瞬間から永久に負のリードタイムを出し続ける。→ `scheduleWindowOccurrence` が、引用された SCHEDULE シグナルの `when` を**予報の発行時刻から見て次に到来する開始時刻**へ解決する（JST 固定＝DST が無いので厳密・複数あれば**早いほう**を採り猶予を長く見せない）。解決できなければ**推定せず、算出していない旨を書く**（[ADR-31](#adr-31-予報の測定は率ではなく落とした件数で出し導けるものは保存しない) の「測っていない」と「0」を混ぜない扱いと同じ）。**時刻の出所は本文に必ず併記する**
- **これは「LLM 出力の構造化を先回りしない」に抵触しない**: [E6-2](../../temp/roadmap-draft/v2-evaluation/todo.md) が構造化を拒否したのは **LLM が生成する `RiskItem.window`**。ここで読むのは `ScheduleSignalSource` が `ScheduleSource` から**そのまま運んだ我々自身の書式**で、LLM を1度も通っていない。⚠ **恒久形は `ScheduleWindow` に機械可読な繰り返し定義を持たせて wire に載せること**（文字列の描画は表示の関心事）。そちらへ移れば `scheduleWindowOccurrence` は丸ごと消える——移行に backend デプロイと予報の再生成が要るので分けた
...
## ADR-34: 予報カードは「決裁の読み順」に並べ替え、「いつまでに動くか」を軸で出す

- **決定**: 予兆リスクカードの構成を **「何が（subject 見出し）→ いつまでに（時間軸）→ 何をする（先手）→ なぜ（収束ミニフロー）→ 詳しく（AI の推論・畳む）→ 証拠（引用）」** に組み替える。あわせて `ForecastTimeline` を新設し、**いま／対処を始める期限／予測発生窓**の3点を線形の軸で描く。ページ上部のメタチップ3つ（対象 / 生成 / 評価シグナル）は一覧見出しへ畳む。
- **理由**: カードは `window` で「いつ危ないか」は言っていたが、**対処を始める期限**をどこにも出しておらず、有効リードタイム（[ADR-33](#adr-33-予報は-pr-コメントで届けるgate-にはせず該当は引用一致を第一根拠にする) / E6-2）の引き算が読み手の暗算に残っていた。決裁の場で先に要るのは**打ち手**であって、正当化はその後に検算されるもの——旧順（入力→AI推論→結論→先手）は物語の順で、判断の順ではない。
- **撤回した過去の決定を3つ明記する**:
  1. **「タイムチャートは不採用」**（理由: `window` が LLM 由来の自由文字列で時刻を捏造せずには描けない）→ **引用された `SCHEDULE` シグナルから決定論で解決できる**ようになり根拠が消えた（`scheduleWindowOccurrence`）。⚠ **読むのは引用シグナルの `when`（我々のスケジュール定義）で、`risk.window` は今も読まない**
  2. **「`window` を主見出し、subject は補足行」** → 軸が「いつ」を**日付つき**で答えるようになり、曜日しか持たない見出しが軸の下位互換になった。3秒で読まれる見出しは「**何が**」が担う。軸を描けないリスクでは `window` を補足行に残す（縮退しても「いつ」は消えない）
  3. **バッジ文言 `HIGH`** → 収束ミニフローの結論ノードが「高リスク」で、**同じ level を2語で言っていた**。`riskLevelLabel` に統一
- **描画の規律**: 線形スケールのまま描く（対処30分が103時間の軸で細いのは事実で、「直す時間に対して判断の時間が圧倒的に長い」が結論そのもの。太らせると間に合う/間に合わないの見え方を歪める）／宣言値は斜線・実測由来はベタ塗り／終了時刻が書かれていない窓は幅を主張しない／**予測発生時刻の出所を画面に出す**（出所の無い時間は材料にならない）／引用にスケジュールが無ければ軸ごと出さない
