...
- **決定**: 予兆（Forecast）は時系列予測・統計 ML を使わず、「既知の未来シグナル（未マージ PR / 未適用 plan / 負荷予定）×過去の記憶」を LLM が突合する推論として構成し、出力の citations を実在シグナル id へ機械照合する。
...
- **恒久策（コンテスト後の課題）**: 本質は「統括と JSON 化の分離」＝熟考する仕事と JSON を書き出す仕事を同じターンでやらせない。`responseSchema`（構造化出力）は制約付きデコードなので空応答/散文が原理的に出ないが、ツール呼び出しと同一呼び出しで併用不可のため、コーディネーターに直接は付けられない。実装形は2つ:
  1. **finalizer 方式**: エージェントループ終了後に、ツールなし・思考最小・`responseSchema` 強制の単発 LLM 呼び出しを直列に足し、セッションの調査結果（sub-agent の出力群）を入力に JSON 化だけをやらせる。エージェント数は増えない（グラフ外の直列化ステップ）ため「8エージェント」の提出物語彙（UI 台帳・README・ポスター・動画フレーム）と衝突しない。
  2. **report_writer サブエージェント方式**: スキーマ強制付き（`outputSchema`＝ツール不要の清書役なので併用制約に当たらない）の sub-agent を作り、コーディネーターに最後に必ず呼ばせる。ランナー側は**コーディネーターの最終テキストではなく、report_writer のツール応答をイベントストリームから直接回収**する（現行の `isFinalResponse` 依存をやめる）。こうするとコーディネーターの最終ターンが空でも無害になる。コーディネーターのプロンプト変更が小さくリスクが低い一方、エージェントが9体になり「8エージェント」を謳う提出物・UI と結合する（採用時は語彙の一斉更新が必要）。
